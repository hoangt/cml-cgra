...
bool REGI_Node:: is_Address_Assert()
{
...
bool REGI_Node::is_ready_to_schedule_Feasible_ALAP(int &time, int MAX_Schedule, int II)
{
    time = MAXINT;
    int current_time;
    vector<REGI_Node*> next_nodes = Get_Successors_Same_Iteration_Exclude_Address_Dependency();
...
//	    printf("Current Node: %d\tNext Node: %d Current_time: %d\ttime: %d\n",get_ID(),next_nodes[i]->get_ID(),current_time, time); 	
    ...
    {
        time = get_Sched_Info()->get_Feasible_ASAP() + II - latency; // + (MAX_Schedule/2);
            //MAX_Schedule - latency;
        return true;
    }
}
