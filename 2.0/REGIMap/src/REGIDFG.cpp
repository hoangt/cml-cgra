...
/*
   if(i == 0) {
	for (int j = 0; j < (int) _ARC_Set.size(); j++)
   	{
	    cout<<"\n"<<"_ARC_Set["<<j<<"]->get_From_Node()->get_ID() = "<<_ARC_Set[j]->get_From_Node()->get_ID();
	    cout<<"\t"<<"_ARC_Set["<<j<<"]->get_To_Node()->get_ID() = "<<_ARC_Set[j]->get_To_Node()->get_ID();
	    cout<<"\t"<<"_ARC_Set["<<j<<"]->get_Number() = "<<_ARC_Set[j]->get_Number();
	    //cout<<"\t"<<"_ARC_Set["<<j<<"]->get_Distance() = "<<_ARC_Set[j]->get_Distance();		
	    //cout<<"\t"<<"_ARC_Set["<<j<<"]->get_Dependency() = "<<_ARC_Set[j]->get_Dependency();
	}
   }  	
    cout<<"\n"<<"At beginning of the Loop";
    cout<<"\n"<<"i = "<<i<<"  _ARC_Set.size() = "<<_ARC_Set.size();
    cout<<"\n"<<"_ARC_Set[i]->get_From_Node()->get_ID() = "<<_ARC_Set[i]->get_From_Node()->get_ID();
    cout<<"\n"<<"_ARC_Set[i]->get_To_Node()->get_ID() = "<<_ARC_Set[i]->get_To_Node()->get_ID();
*/			
...
void REGI_DFG::Apply_Outdegree_Constraint()
{
  ...
  for (int i = 0; i < (int) _node_Set.size(); i++)
  {
    ...
    {
      //cout << "\nNode Requires Outdegree Constraint: " << startNode->get_ID();
	...
	//cout << "\nAdding node in between for node: " << startNode->get_ID();
	...
	//cout << "\nRemoving arc between node: " << startNode->get_ID() << " and " << (*iNode1)->get_ID();
        Remove_Arc(startNode, *iNode1);
        make_Arc(newnode, *iNode1, ARC_Max_Index++, distance, arc->get_Dependency());
	//cout << "\nMaking arc between node: " << newnode->get_ID() << " and " << (*iNode1)->get_ID();
 ...
 */
int REGI_DFG::Init_Schedule(int id, int number_of_resources, int rec_MII)
{
   ...
   */
  int MemII = (Count_Load_Operations()  + Count_Store_Operations() - 1) / CGRA_Y_Dim;
  ...
  vector<REGI_Node*> set = _node_Set;
  ofstream myfile;

  myfile.open("/home/shail/CGRA/mywork/schedule/ASAP_DFG_0.txt");
  
  myfile << "\nASAP (Init_Schedule) Details For DFG: \n";

  for (int i = 0; i < (int) set.size(); i++)
  {
    myfile << set[i]->get_ID() << "\t" << set[i]-> get_Sched_Info()->get_ASAP() << "\n";
  }
  ...
  //Shail
  myfile.open("/home/shail/CGRA/mywork/schedule/ALAP_DFG_0.txt");
  
  myfile << "\nALAP (Init_Schedule) Details For DFG: \n";

  for (int i = 0; i < (int) set.size(); i++)
  {
    myfile << set[i]->get_ID() << "\t" << set[i]-> get_Sched_Info()->get_ALAP() << "\n";
  }
  ...
  myfile.open("/home/shail/CGRA/mywork/schedule/ASAP_DFG_1.txt");
  
  myfile << "\nASAP Flexible (Init_Schedule) Details For DFG: \n";

  for (int i = 0; i < (int) set.size(); i++)
  {
    myfile << set[i]->get_ID() << "\t" << set[i]-> get_Sched_Info()->get_Feasible_ASAP() << "\n";
  }
  ...
  myfile.open("/home/shail/CGRA/mywork/schedule/ALAP_DFG_1.txt");
  
  myfile << "\nALAP Flexible (Init_Schedule) Details For DFG: \n";

  for (int i = 0; i < (int) set.size(); i++)
  {
    myfile << set[i]->get_ID() << "\t" << set[i]-> get_Sched_Info()->get_Feasible_ALAP() << "\n";
  }
...
int REGI_DFG::Schedule(int id, int number_of_resources, int II)
{
  ...
  myfile.open("/home/shail/CGRA/mywork/schedule/ALAP_DFG_2.txt");
  
  myfile << "\nALAP Flexible (Schedule) Details For DFG: \n";
  ...
  while (true)
  {
     ...
     */
    //cout << "\n\nModulo_Schedule_Smart_2 called\n\n";
    //cout << "last_time = " << last_time << endl;
	...
	//Shail
      printf("Feasible II is %d\tSchedule Len is %d\n",II,schedule_len);	
    ...
    if (redo)
    {
      ...
      last_time = Schedule_ASAP_Feasible(number_of_resources);
      MAX_SCHEDULE_LEN = last_time;
      //cout << "MAX_SCHEDULE_LEN = " << last_time << endl;
      /*
...
int REGI_DFG::Schedule_ASAP_Feasible(int number_of_resources)
{
  ...
  vector<REGI_Node*> rest = _node_Set;
  vector<REGI_Node*> not_scheduled;
  /*
  ...
  for (int i = 0; i < (int) to_be_scheduled.size(); i++)
  {
    ...
    {
        //Store Nodes Can Not Be There At Beginning
        /*
 ...
 * FIXME: 20-2-2014
 */
bool REGI_DFG::Schedule_ALAP_Feasible(int latesttime, int number_of_resources, int II)
{
  ...
  while ((int) rest.size() > 0)
  {
    ...
    for (int i = 0; i < (int) rest.size(); i++)
    {
      ...
      if (rest[i]->is_ready_to_schedule_Feasible_ALAP(schedule_time, MAX_SCHEDULE_LEN, II))
      {
        ...
        //Shail
//	printf("\nNode %d is ready to be scheduled at time %d", rest[i]->get_ID(),schedule_time);
...
/*
  for(int i = 0; i < to_be_scheduled.size(); i++)
  { 
	cout << "To be scheduled: " << i << "\tNode: " << to_be_scheduled[i]->get_ID() << endl;
  }
*/
  ...
  /*  for(int i = 0; i < ready_sorted.size(); i++)
    { 
	cout << "Ready Sorted: " << i << "\tNode: " << ready_sorted[i]->get_ID() << endl;
    }
*/
    ...
    //cout << "Node = " << ready_sorted[0]->get_ID() << "\tSchedule_time = " << schedule_time << "\tEnd = " << end << endl;
...
void REGI_DFG::Update_Minimizable_Nodes()
{
  ...
  while (true)
  {
    ...
    for (vector<REGI_Node*>::const_iterator it = _node_Set.begin(); it!= _node_Set.end(); ++it)
    {
      ...
      {
        (*it)->set_Minimizable();
        change = false;             //Shail //Previously it was true
        continue;
      }
...
void REGI_DFG::Reduce_DFG(int rec_MII)
{
  while (true)
  {
    Init_Schedule(0, CGRA_Y_Dim * CGRA_X_Dim, rec_MII);
...
string REGI_DFG::Generate_Latency()
{
  ofstream myfile;
