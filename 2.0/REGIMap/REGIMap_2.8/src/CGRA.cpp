    ...
    for (int index1 = 0; index1 < tableSize; index1++)
    {
      ...
      for (int index2 = 0; index2 < (int) cons1.size(); index2++)
      {
        ...
        for (int index3 = 0; index3 < tableSize; index3++)
        {
          ...
          //a successor mapped to the same PE, update number of registers
          //required
...
void CGRA::Prolog(REGI_DFG* myDFG)
{
  ...
  INTERITERATIONDEPENDENCY Dependency;
  int highest_distance = Dependency.get_highest_distance_from_inter_iteration_dependencies();
  ...
  /* Now We Should Ensure That Each Phi Node Is Present In Prolog As Many Times As Their Loop-Carried Dependency Weight
   * For This, We Find Highest Weight From All Loop-Carried Dependencies
   * Then, We Unroll Kernel That Many Times
   */

  //cout << "Unroll Kernel = " << unroll_kernel << endl;
  int unroll_time = t + 1;
  int unroll_count = 0;

  //We should see whether all phi nodes are present at least once. If not then we need to unroll one more time.
  // Start with unroll_count as 1 else.

  int unroll_kernel = 1; //highest_distance;

  while(unroll_count < unroll_kernel)
  {
        int timeIndex = 0;
	for (timeIndex = 0; timeIndex < II; timeIndex++)
	{
	    TimeCGRA* cgraTime = timeCGRAList[timeIndex];
	    REGI_PE* candidatePE;
	    cout << "Time:" << timeIndex + (unroll_time) << endl;
	    for (int i = 0; i < SizeX; i++)
	    {
	      for (int j = 0; j < SizeY; j++)
	      {
  	...
  	unroll_time = unroll_time + timeIndex;
